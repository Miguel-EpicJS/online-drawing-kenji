...
ws.onmessage = (ms) => {
  ...
  if (submitedData.action === "get-first") {
    document
      .getElementById(submitedData.msg.text.id)
      .classList.replace("players-list", "playing");
  }

  function showMessage(message, whichPlayer) {
    if (whichPlayer == "me") {
  ...
  sendBtn.onclick = function () {
    if (!ws) {
      showMessage("No websocket connection :(", "me");
      return;
    }
    let userObj = {
      nick: "",
  ...
  if (submitedData.path === "/chat") {
    showMessage(submitedData.msg.text, "otherPlayer");
  }
